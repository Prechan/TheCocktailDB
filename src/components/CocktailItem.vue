<template>
  <div
    ...
    @click="modalTransition($event)"
    ref="cocktailItemWrapper"
    :style="styleObject"
  >
    <slot name="image"></slot>
    <div
      class="transition-all mx-auto p-3 relative bottom-8 bg-white w-4/5 sm:w-11/12 md:w-4/5 border border-black ingredients"
    >
      ...
      <img
        class="hidden sm:block sm:w-6 sm:mx-auto sm:my-2 transition-all"
        :class="recipeIsVisible ? 'rotate-180' : ''"
        src="./icons/downArrow.svg"
        alt="arrow"
        @click="displayIngredients"
      />
      <p
        class="recipe text-sm transition-all"
        :class="
          recipeIsVisible
            ? 'opacity-100 scale-100 h-100'
...
export default {
  ...
  methods: {
    modalTransition() {
      ...
      let viewportOffset =
        this.$refs["cocktailItemWrapper"].getBoundingClientRect();

      // Si la carte est ouverte...
      if (
        this.$refs["cocktailItemWrapper"].classList.contains("open") &&
